...
contract TxnSimpleMarketV2 is TxnMarketPermissionSupportV1 {
    ...
    function _calculateChainPrice(uint128 goodsId_, uint128 amount, uint16 stableId) internal view returns (uint256 total, uint256 rate){
        IMintableMarket.GoodsInfo memory goods = _mintableMarket.goodsInfo(goodsId_);
        uint128 goodsPrice = goods.price;
        (uint _rate, uint decimals) = currentStableExchangeRate(stableId);
    ...
    function mintPriceEstimate(uint128 goodsId_, uint128 amount) external view returns (uint256 total) {
        (uint256 totalFee,) = _calculateChainPrice(goodsId_, amount, uint16(0));
        return totalFee;
    }

    function mintPriceStableEstimate(uint128 goodsId_, uint128 amount, uint16 stableId) external view returns (uint256 total) {
        (uint256 totalFee,) = _calculateChainPrice(goodsId_, amount, stableId);
        return totalFee;
    }
    ...
    function customerMint(uint128 goodsId_, uint128 amount) external payable {
        (uint256 totalPrice, uint256 rate) = _calculateChainPrice(goodsId_, amount, uint16(0));
    ...
    function customerMintStable(uint128 goodsId_, uint128 amount, uint16 stableId) external {
        ...
        (uint256 totalPrice, uint256 rate) = _calculateChainPrice(goodsId_, amount, stableId);
    ...
    function _buyTokenEstimate(address seller_, address nftContractAddr_, uint128 tokenId_, uint32 amount_, uint16 stableId_) internal view returns (uint256 total, uint256 rate) {
        (, uint64 price) = _nftSaleMarket.querySaleStatus(seller_, nftContractAddr_, tokenId_);
        (uint256 rate_,uint8 decimals) = currentStableExchangeRate(stableId_);
    ...
    function buyTokenEstimate(address seller_, address nftContractAddr_, uint128 tokenId_, uint32 amount_) public view returns (uint256 total) {
        (uint256 totalPrice,) = _buyTokenEstimate(seller_, nftContractAddr_, tokenId_, amount_, uint16(0));
        return totalPrice;
    }

    function buyTokenWithStableEstimate(address seller_, address nftContractAddr_, uint128 tokenId_, uint32 amount_, uint16 stableId_) public view returns (uint256 total) {
        (uint256 totalPrice,) = _buyTokenEstimate(seller_, nftContractAddr_, tokenId_, amount_, stableId_);
        return totalPrice;
    }
    ...
    **/
    function buyToken(address payable seller_, address nftContractAddr_, uint128 tokenId_, uint32 amount_) external payable {
        (uint256 totalPrice, uint256 rate) = _buyTokenEstimate(seller_, nftContractAddr_, tokenId_, amount_, uint16(0));
    ...
    function buyTokenStable(address payable seller_, address nftContractAddr_, uint128 tokenId_, uint32 amount_, uint16 stableId_) external {
        ...
        (uint256 totalPrice, uint256 rate) = _buyTokenEstimate(seller_, nftContractAddr_, tokenId_, amount_, stableId_);
        require(_msgSender() != seller_, "Market: You are selling to yourself");
        require(amount_ > 0, "Market: Must buy at least 1");
        ...
        uint256 evtAmount = stableId_ << 240;
    ...
    function currentStableExchangeRate(uint16 stableId) public view returns (uint256 rate, uint8 decimals) {
